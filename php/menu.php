...
foreach ($menu as $dept=>$dcat) {
    ...
    $htm .= '              <a href="index.php?dept='.$d.'">'.str_replace('_',' ',ucwords($dept)).'</a>';
    ...
    if (isset($dcat)) {
		...
		foreach ($dcat as $catg=>$citm) {
			...
			$htm .= '                  <a href="index.php?dept='.$d.'&catg='.$c.'">'.str_replace('_',' ',ucwords($catg)).'</a><span id="catg-disc-'.$d.'-'.$c.'-0" class="catg-disc"></span>'.PHP_EOL;
			...
			foreach ($citm as $item) {
				...
				$htm .= '                    <a href="index.php?dept='.$d.'&catg='.$c.'&item='.$i.'">'.str_replace('_',' ',ucwords($item)).'</a>'.PHP_EOL;
